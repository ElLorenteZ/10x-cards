...
**Project-specific rules live in @AGENTS.md** — stack & runtime, hard rules (e.g. `prerender = false` on API routes), auth flow, conventions, env setup, and deploy. It is the single source of truth; read it first. Do not duplicate those rules here.
